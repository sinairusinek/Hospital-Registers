...
- Years were widely misread, a page at a time, and `build.py` repairs them from the register's own order: 360 admissions that ran backwards against their notebook's sequence, 17 discharge years standing a year over the clerk's own count, and 5 dates outside 1930–48 repaired from the other date on the record. The two notebook 8 `17/5/63` records that once stretched the range to 1963 are now resolved, and the normalized date range is a clean **1930–1948**; no `date-out-of-span` flag remains. One `01/01/1900` survives in `Discharge Date as written` (notebook 9, record 1232) — a spreadsheet epoch zero, not a date; the corrected column is clean.
